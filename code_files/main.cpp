...
vector<string> find_valid_files()  //finds tsp data files in the desired data format 
{
    ...
    while ((dp = readdir(dirp)) != NULL) 
    {
        ...
        {   
            if(validate_file("ALL_TSP_new/"+fname))
...
vector<int> _2opt_step(vector<int> ham_path, int x1, int x2, int x3, int x4)  //induces a 2 opt exchange with (x1,x2,x3,x4) (needs to be a cyclic sequence)
{
    int n=ham_path.size();
...
float scatter(vector<vector<float> > adj_mat, vector<int> ham_path, int n, int* min_node)  //finds the scatter (min edge) of a path
{
...
int main()  //start
{
    ...
    float maxima, minima, beta=0.5;  //0.05,0.5 (*)
    ofstream MyFile("final_results.txt");
    ...
    for(emsun=0; emsun<=1;emsun+=1)
    {
        for(randomness=1; randomness<=2; randomness+=1)
        {
            ...
            {
        //        valid_fnames[i]="ulysses16.tsp";  //easy sample test file
                nodes = read_data_file("ALL_TSP_new/"+valid_fnames[i],&maxima);  //read data-coordinates
                nodes_size=nodes.size();
                if(nodes.size()>2000)  //7500 (*)
                {
                ...
                cout<<"emsun: "<<emsun<<" ;std_dev_e: "<<std_dev_e<<','<<"; std_dev_s: "<<std_dev_s<<endl;
        //        std_dev=beta*minima;  //(*), beta*min_dist between any pair of nodes
        //        std_dev=0.5*maxima/sqrt(nodes_size*log(nodes_size));  //(*), max square width*(1/(2*sqrt(n*logn)))
        //        b=beta*minima;  //(*), beta*min_dist between any pair of nodes
                if(emsun==0)
                {
                    std_dev=std_dev_s;
                    a=-std_dev_s;
                    b=std_dev_s;  //(*), max square width*(1/(2*sqrt(n*logn)))
                }
                else if(emsun==1)
                {
                    std_dev=std_dev_e;
                    a=-std_dev_e;
                    b=std_dev_e;  //(*), beta*min_dist between any pair of nodes
                }                    
                if (randomness==1)
                {
        ...
        //        cout << "uniform: "<<b<<endl;  //(*)
        //        MyFile <<"uniform: "<<b<<endl;  //(*)
                for(int k=0;k<counter;k++)
                {
                    scatter_bound = pow(2,16)-1;  //INFINITE initial scatter bound (min among all 2nd max edge of all vertices)
                    if (randomness==1)
...
//                   MyFile << "results: "<<naive_greedy_result<<','<<naive_weave_result<<','<<hoffmann_weave_result<<endl;
//                    MyFile << "times: "<<time_taken[0]<<','<<time_taken[1]<<','<<time_taken[2]<<endl;
                }
        //        if(i==5)
        //            exit(0);
            }
...
//check out g-cloud, parallelism using gpus, data storing format